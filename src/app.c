...
static _attribute_ram_code_ bool should_display_ext_data_layout() {
...
static _attribute_ram_code_ void display_ext_data_layout(void) {
...
static inline bool should_display_temperature_battery_layout() {
#if	HAS_BATTERY_LEVEL_DISPLAY
...
static _attribute_ram_code_ void display_temperature_battery_layout() {
	display_temperature();
	show_battery_symbol(1);
	show_smiley(0);
	...
	show_small_number_x10(battery_level, !HAS_BATTERY_LEVEL_DISPLAY);
#else
	show_small_number((battery_level >= 100) ? 99 : battery_level, !HAS_BATTERY_LEVEL_DISPLAY);
#endif
}

static _attribute_ram_code_ uint8_t get_smiley(int16_t t, int16_t h) {
...
static _attribute_ram_code_ void display_temperature_humidity_layout() {
	display_temperature();
	show_battery_symbol(HAS_BATTERY_LEVEL_DISPLAY);
...
static _attribute_ram_code_ bool display(void) {
	static RAM uint8_t available_layouts = 0xFF;
	show_ble_symbol(ble_connected);
	if ((available_layouts & DISPLAY_EXT_DATA) && should_display_ext_data_layout()) {
		available_layouts ^= DISPLAY_EXT_DATA;
		display_ext_data_layout();
	} else if ((available_layouts & DISPLAY_TEMP_BAT) && should_display_temperature_battery_layout()) {
	...
	} else if ((available_layouts & DISPLAY_CLOCK) && cfg.flg.blinking_time_smile) {
...
_attribute_ram_code_ void lcd(void) {
	if (!display())
		display(); // Ensure that we update the display
}
